...
ipaddress_owners=($(cat "$base_directory/owner_devices" | grep -oiE "([0-9]{1,3}\.?){4}" ))
[ -z "$ipaddress_owners" ] && echo "warning: no owner devices are specified. exiting." && exit 1
...
scan_arp () {
	if [ ! -z "$1" ]; then
		local result=$(sudo arping -c 5 "$1" 2>&1 | grep 'from' )
		debug_echo "Scan result: [$result]"
		echo "$result"
	fi
}
